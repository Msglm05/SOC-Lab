...
🔔 I had trouble with the Kali Linux machine with regard to the xfreerdp command, the which xfreerdp command returned 'xfreerdp not found' although the freerdp3-x11 packages were present. The crowbar command was not able to sucessfully execute so I decided to pivot to a Ubuntu machine.
...
3. Once it has been set, open up the terminal and run ip a, to verify that the settings have been applied, you can additionaly ping any website to see if you have connectivity and can ping by domain names to see DNS is working, make sure your DC is on at this point❗
